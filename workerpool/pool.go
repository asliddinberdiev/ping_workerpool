...
func (r Result) Info() string {
	...
	return fmt.Sprintf("[SUCCESS] - [%s] - STATUS_CODE: %d, RESPONSE_TIME: %s", r.URL, r.StatusCode, r.ResponseTime)
}
...
func (p *Pool) initWorker(id int) {
	for job := range p.jobs {
		...
		p.results <- p.worker.proccess(job)
		p.wg.Done()
	}

	log.Printf("[WORKER_ID: %d] finished proccessing\n", id)
}
